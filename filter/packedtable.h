...
namespace hashfilter {
    ...
    public:
        ...
        PackedTable() {
            len = (size_t) ceil(bits_per_bucket * num_buckets / 8.0);
            bytes_per_bucket = (size_t) ceil(bits_per_bucket / 8.0);
            buckets_ = new char[len];
            CleanupTags();
        }
        ...
        void CleanupTags() { memset(buckets_, 0, len ); }

        size_t SizeInTags() const { return 4 * num_buckets; }

        size_t SizeInBytes() const { return len; }
        ...
        void PrintTags(uint32_t tags[4]) const {
            ...
            uint16_t codeword = perm.encode(lowbits);
        ...
        inline void ReadBucket(const size_t i, uint32_t tags[4]) const {
            ...
            uint16_t v = perm.dec_table[codeword];
            lowbits[0] = (v & 0x000f);
...
//            DPRINTF(DEBUG_TABLE, "PackedTable::FindTagInBucket %zu\n", i);
            ...
            uint16_t v;
            uint64_t  bucketbits1 = *((uint64_t*) (buckets_ + 6 * i1 ));
            uint64_t  bucketbits2 = bucketbits1; //*((uint64_t*) (buckets_ + 6 * i2 ));
            ...
            v = perm.dec_table[(bucketbits1 ) & 0x0fff];
            tags1[0] |= (v & 0x000f); 
            tags1[1] |= ((v>>4) & 0x000f);
            tags1[2] |= ((v>>8) & 0x000f);
            tags1[3] |= ((v>>12) & 0x000f);
            ...
            v = perm.dec_table[ (bucketbits2 ) & 0x0fff];
            tags2[0] |= (v & 0x000f); 
            tags2[1] |= ((v>>4) & 0x000f);
            tags2[2] |= ((v>>8) & 0x000f);
            tags2[3] |= ((v>>12) & 0x000f);
